...
- `log_adapter_supported`
- `evidence_signals`（最多 5 条）

Agent context 只返回路径和检测证据摘要，不读取配置目录内容，不执行 CLI，不启动 App。
...
Agent proposal 支持以下 `proposal_type`：
...
Agent proposal 是 review-only 建议，不会被 Trust Policy 自动应用。
...
Agent proposal 不参与自动应用。Codex 可以创建建议，但确认、忽略、启动、路径修改等动作仍必须由 Harness UI 和后续受控工作流处理。
