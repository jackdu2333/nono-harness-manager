  ...
  const PathRow = ({ label, path, canOpen }: { label: string, path: string | null, canOpen?: boolean }) => (
    <div className="flex items-center justify-between gap-4 py-2.5 border-b border-border/50 last:border-0 group">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs font-mono text-foreground truncate" title={path || t('agents.not_configured')}>
          {path || <span className="text-muted-foreground italic">{t('agents.not_configured')}</span>}
        </div>
      </div>
      <div className="shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {path && canOpen && (
          <Button variant="ghost" size="icon" className="w-6 h-6" onClick={() => openConfigDir(agent.id)} title={t('agents.open_in_finder')}>
    ...
    <div className="space-y-6">
      <section>
        ...
        <div className="flex gap-3 text-sm text-foreground/90 leading-relaxed bg-primary/5 p-4 rounded-lg border border-primary/10">
          ...
          <p>
            {agent.description || t('agents.feature_desc_template', { type: agent.type || t('common.unknown') })}
          </p>
        </div>
      </section>
