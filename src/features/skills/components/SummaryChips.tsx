...
export function SummaryChips() {
  const { skills, currentView, setView, duplicateAssignment } = useSkillsStore();

  const active = skills.filter((s) => s.is_archived === 0);
  const dupCount = active.filter((s) => !!duplicateAssignment[s.id]).length;
  ...
  const chips: { view: SkillView; label: string; count: number }[] = [
    ...
    { view: 'archived', label: '已归档', count: skills.filter((s) => s.is_archived === 1).length },
  ];

  return (
    <div className="shrink-0 flex items-center gap-2 px-4 lg:px-6 py-2 border-b border-border bg-background overflow-x-auto">
      {chips.map((chip) => {
