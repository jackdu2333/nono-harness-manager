    ...
    <div className="h-full overflow-auto bg-background p-6">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">本地记忆目录索引、文件统计与只读健康检查。</p>
        </div>
      ...
      <section className="mb-5 grid grid-cols-[1fr_1.4fr] gap-3">
        <Input value={name} onChange={event => setName(event.target.value)} placeholder="记忆目录名称" />
        <div className="flex gap-2">
          <Input value={path} onChange={event => setPath(event.target.value)} placeholder="记忆根目录路径" />
          <select
            value={memoryType}
          ...
          >
            ...
            <option value="temporary">临时记忆</option>
            <option value="archive">归档记忆</option>
          </select>
          <Button onClick={addSource} disabled={isLoading || !path.trim()}>添加</Button>
        </div>
      </section>

      <div className="grid grid-cols-[360px_1fr] gap-5">
        <section className="min-h-[520px] border border-border bg-card">
          <div className="border-b border-border px-4 py-3 text-sm font-semibold">记忆根目录</div>
          {sources.length === 0 ? (
            <div className="px-4 py-6 text-sm text-muted-foreground">尚未添加记忆目录。</div>
          ) : (
            sources.map(source => (
              <button
                key={source.id}
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {source.file_count} 文件 · {formatBytes(source.total_size_bytes)}
                </div>
              </button>
            ))
          )}
        ...
        <section className="min-h-[520px] border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <div>
              <div className="text-sm font-semibold text-foreground">{selected?.name ?? '文件列表'}</div>
              <div className="mt-1 text-xs text-muted-foreground">
                最近修改：{formatTime(selected?.last_modified_at)}
              </div>
            </div>
            {selected?.path && (
          ...
          {health && (
            <div className="border-b border-border px-4 py-3 text-sm">
              ...
              <span className="ml-3 text-muted-foreground">问题 {health.issues.length} 条</span>
            </div>
          )}

          {!files ? (
            <div className="px-4 py-6 text-sm text-muted-foreground">选择一个记忆目录查看文件。</div>
          ) : (
