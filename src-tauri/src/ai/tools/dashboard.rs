...
pub async fn get_dashboard_summary(ctx: &ToolContext<'_>) -> Result<ToolOutput, String> {
    ...
    let skills_needs_review = sqlx::query("SELECT COUNT(*) as cnt FROM skills WHERE needs_review = 1")
        .fetch_one(ctx.pool).await.map_err(|e| e.to_string())?
        .get::<i64, _>("cnt");

    let skills_needs_improvement = sqlx::query("SELECT COUNT(*) as cnt FROM skills WHERE needs_improvement = 1")
        .fetch_one(ctx.pool).await.map_err(|e| e.to_string())?
        .get::<i64, _>("cnt");
    ...
    let agents_candidate = sqlx::query("SELECT COUNT(*) as cnt FROM agents WHERE is_user_confirmed = 0 AND is_ignored = 0")
        .fetch_one(ctx.pool).await.map_err(|e| e.to_string())?
        .get::<i64, _>("cnt");
    ...
    let proposals_pending = sqlx::query("SELECT COUNT(*) as cnt FROM intelligence_proposals WHERE status = 'pending'")
        .fetch_one(ctx.pool).await.map_err(|e| e.to_string())?
        .get::<i64, _>("cnt");

    let proposals_applied = sqlx::query("SELECT COUNT(*) as cnt FROM intelligence_proposals WHERE status = 'applied'")
        .fetch_one(ctx.pool).await.map_err(|e| e.to_string())?
        .get::<i64, _>("cnt");

    let proposals_rejected = sqlx::query("SELECT COUNT(*) as cnt FROM intelligence_proposals WHERE status = 'rejected'")
        .fetch_one(ctx.pool).await.map_err(|e| e.to_string())?
        .get::<i64, _>("cnt");
