    ...
    for result in walker {
        if let Ok(entry) = result {
            if entry.file_type().map_or(false, |ft| ft.is_file()) {
                ...
                let parent_dir = entry.path().parent().unwrap_or(Path::new("")).to_string_lossy().to_string();

                if file_name == "agent.json" || file_name == "config.yaml" || file_name == "mcp.json" || file_name == "settings.json" || file_name == "mcp_config.json" {
...
pub fn auto_discover_agents() -> Vec<Agent> {
    ...
    if cursor_app.exists() {
        ...
        add_agent!("", "Cursor", "IDE", "/Applications/Cursor.app", Some("open -a Cursor".to_string()), if config_path.exists() { Some(config_path.to_string_lossy().to_string()) } else { None });
    }
    ...
    if claude_app.exists() {
        ...
        add_agent!("", "Claude Desktop", "App", "/Applications/Claude.app", Some("open -a Claude".to_string()), if config_path.exists() { Some(config_path.to_string_lossy().to_string()) } else { None });
    }
    ...
    if windsurf_app.exists() {
        ...
        add_agent!("", "Windsurf", "IDE", "/Applications/Windsurf.app", Some("open -a Windsurf".to_string()), if config_path.exists() { Some(config_path.to_string_lossy().to_string()) } else { None });
    }
    ...
    if codex_dir.exists() {
        ...
        add_agent!("", "Codex", "CLI", codex_dir.to_string_lossy().to_string(), None, if config_path.exists() { Some(config_path.to_string_lossy().to_string()) } else { None });
    }

    // 5. Claude Code CLI
    let claude_cli_config = home.join(".claude.json");
    if claude_cli_config.exists() {
        add_agent!("", "Claude Code", "CLI", home.join(".claude").to_string_lossy().to_string(), None, Some(claude_cli_config.to_string_lossy().to_string()));
    }

    // 6. Workbuddy
    let workbuddy_dir = home.join("Library/Application Support/WorkBuddyExtension");
    if workbuddy_dir.exists() {
        add_agent!("", "Workbuddy", "App", workbuddy_dir.to_string_lossy().to_string(), None, None);
    }
            ...
            if let Ok(file_type) = entry.file_type() {
                if file_type.is_dir() {
                    ...
                    let config_path = if dir_path.join("settings.json").exists() {
                        ...
                        Some(dir_path.join("mcp_config.json").to_string_lossy().to_string())
                    ...
                    if config_path.is_some() || dir_name.starts_with("antigravity") {
                        discovered.push(Agent {
                            id: Uuid::new_v4().to_string(),
                            name: pretty_name,
                            r#type: Some(if dir_name.contains("cli") { "CLI".to_string() } else if dir_name.contains("ide") { "IDE Plugin".to_string() } else { "Agent".to_string() }),
