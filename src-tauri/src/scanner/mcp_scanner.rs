    ...
    for result in walker {
        if let Ok(entry) = result {
            if entry.file_type().map_or(false, |ft| ft.is_file()) {
                ...
                if file_name == "package.json" {
                    if let Ok(content) = fs::read_to_string(entry.path()) {
                        if let Ok(json) = serde_json::from_str::<Value>(&content) {
                            let has_mcp = json.get("dependencies")
                                .and_then(|d| d.get("@modelcontextprotocol/sdk"))
                                .is_some();
                                
                            if has_mcp {
                                let name = json.get("name").and_then(|v| v.as_str()).unwrap_or("Unnamed MCP").to_string();
                                ...
                                discovered.push(McpServer {
                                    ...
                                    args: Some(serde_json::to_string(&vec!["build/index.js"]).unwrap()),
                                    env: None,
                ...
                else if file_name == "mcp.json" || file_name == "mcp_config.json" || file_name == "claude_desktop_config.json" {
                    if let Ok(content) = fs::read_to_string(entry.path()) {
                        if let Ok(json) = serde_json::from_str::<Value>(&content) {
                                ...
                                for (name, config) in mcp_servers {
                                    ...
                                    if !command.is_empty() {
                                        discovered.push(McpServer {
                                            id: Uuid::new_v4().to_string(),
                                            name: name.clone(),
                                            description: Some(format!("From {}", entry.file_name().to_string_lossy())),
...
pub fn discover_system_mcp() -> Vec<McpServer> {
    ...
    let config_paths = vec![
        ...
        home.join("Library/Application Support/WorkBuddyExtension/Data/default/VSCode_mcp_oauth.json"),
                ...
                if let Some(mcp_servers) = json.get("mcpServers").and_then(|v| v.as_object()) {
                    for (name, config) in mcp_servers {
                        ...
                        if !command.is_empty() {
                            ...
                            let source_app = if config_path.to_string_lossy().contains("Claude") { "Claude Desktop" }
                                else if config_path.to_string_lossy().contains(".claude.json") { "Claude Code CLI" }
                                else if config_path.to_string_lossy().contains("QoderCN") { "QoderCN" }
                                else if config_path.to_string_lossy().contains("CherryStudio") { "CherryStudio" }
                                else if config_path.to_string_lossy().contains("WorkBuddy") { "WorkBuddy" }
                                else if config_path.to_string_lossy().contains(".codex") { "Codex" }
                                else if config_path.to_string_lossy().contains(".gemini") { "NoNo Platform" }
                                else { "System Config" };
