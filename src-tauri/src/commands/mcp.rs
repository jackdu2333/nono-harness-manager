...
pub async fn list_mcp_servers(pool: State<'_, SqlitePool>) -> Result<Vec<McpServer>, String> {
    mcp_repository::list_mcp_servers(&*pool)
        .await
        .map(|servers| {
            servers
                .into_iter()
                .filter(|s| s.status.as_deref() != Some("ignored"))
                .collect()
        })
...
pub async fn add_mcp_server(server: McpServer, pool: State<'_, SqlitePool>) -> Result<(), String> {
    mcp_repository::add_mcp_server(&*pool, &server)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn delete_mcp_server(id: String, pool: State<'_, SqlitePool>) -> Result<(), String> {
    mcp_repository::delete_mcp_server(&*pool, &id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn scan_mcp_dir(path: String, pool: State<'_, SqlitePool>) -> Result<usize, String> {
    ...
    for server in new_servers {
        let existing = mcp_repository::list_mcp_servers(&*pool)
            .await
            .unwrap_or_default();
        if let Some(ignored) = existing
            .iter()
            .find(|s| same_mcp_identity(s, &server) && s.status.as_deref() == Some("ignored"))
        {
            let _ = sqlx::query("UPDATE mcp_servers SET status = 'active' WHERE id = ?")
                .bind(&ignored.id)
                .execute(&*pool)
                .await;
        } else if !existing.iter().any(|s| same_mcp_identity(s, &server)) {
...
pub async fn discover_system_mcp(pool: State<'_, SqlitePool>) -> Result<usize, String> {
    ...
    for server in new_servers {
        let existing = mcp_repository::list_mcp_servers(&*pool)
            .await
            .unwrap_or_default();
        if !existing.iter().any(|s| same_mcp_identity(s, &server)) {
            let _ = mcp_repository::add_mcp_server(&*pool, &server).await;
        }
    }
...
fn same_mcp_identity(existing: &McpServer, discovered: &McpServer) -> bool {
    existing.name == discovered.name
        && existing.source_path == discovered.source_path
