...
// HealthIssue / HealthReport 已迁移到 crate::health::types，通过 re-export 暴露
    ...
    for source in &sources {
        ...
        if !root.exists() {
            issues.push(
                ...
                .with_resource(Some(source.name.clone()), Some(path.to_string())),
            );
            continue;
        }
        ...
        for file in files {
            if file.size_bytes as u64 > LARGE_MEMORY_FILE_BYTES {
                issues.push(
                    ...
                    .with_resource(Some(file.name.clone()), Some(file.path.clone())),
                );
            }
        ...
        for (name, count) in seen {
            if count > 1 {
                issues.push(
                    ...
                    .with_resource(Some(name.clone()), Some(path.to_string())),
                );
            }
        }
    }

    // 使用简化评分（memory check 保持向后兼容，不需要 summary）
    let score: i64 = {
        let mut s = 100i64;
        for issue in &issues {
            s -= match issue.severity.as_str() {
                "critical" => 20,
                "error" => 12,
                "warning" => 6,
                _ => 2,
...
pub async fn run_global_health_check(pool: State<'_, SqlitePool>) -> Result<HealthReport, String> {
    crate::health::run_global_check(&pool).await
}
...
// health_report / path_issue / append_*_issues 已迁移到 crate::health 模块
