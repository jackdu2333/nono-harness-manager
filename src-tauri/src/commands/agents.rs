...
pub async fn list_agents(pool: State<'_, SqlitePool>) -> Result<Vec<Agent>, String> {
    agent_repository::list_agents(&*pool)
        ...
        .map(|agents| {
            agents
                .into_iter()
                .filter(|a| a.status.as_deref() != Some("ignored"))
                .collect()
        })
...
pub async fn delete_agent(id: String, pool: State<'_, SqlitePool>) -> Result<(), String> {
    agent_repository::delete_agent(&*pool, &id)
        .await
        .map_err(|e| e.to_string())
}

#[command]
pub async fn scan_agents_in_dir(
    path: String,
    pool: State<'_, SqlitePool>,
) -> Result<usize, String> {
    ...
    for agent in new_agents {
        ...
        if let Some(ignored) = existing
            .iter()
            .find(|a| same_agent_identity(a, &agent) && a.status.as_deref() == Some("ignored"))
        {
            ...
            let _ = sqlx::query("UPDATE agents SET status = 'active' WHERE id = ?")
                ...
                .execute(&*pool)
                .await;
        } else if !existing.iter().any(|a| same_agent_identity(a, &agent)) {
...
pub async fn launch_agent(id: String, pool: State<'_, SqlitePool>) -> Result<(), String> {
    // Find agent
    let agents = agent_repository::list_agents(&*pool)
        .await
        .map_err(|e| e.to_string())?;
    let agent = agents
        .into_iter()
    ...
    if !is_directly_launchable(
        agent.r#type.as_deref(),
        agent.app_path.as_deref(),
        agent.launch_command.as_deref(),
    ) {
...
mod tests {
    ...
    fn cli_and_ide_plugin_agents_are_not_directly_launchable() {
        assert!(!is_directly_launchable(
            Some("CLI"),
            Some("/Users/jackdu/.codex"),
            None
        ));
        assert!(!is_directly_launchable(
            Some("IDE Plugin"),
